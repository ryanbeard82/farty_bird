...
other_sfx_channel = pygame.mixer.Channel(1)
...
# Load Audio
FART_FX = [
    pygame.mixer.Sound(os.path.join("assets/audio/fart_fx","fart_1.wav")),
    pygame.mixer.Sound(os.path.join("assets/audio/fart_fx","fart_2.wav")),
    pygame.mixer.Sound(os.path.join("assets/audio/fart_fx","fart_3.wav")),
    pygame.mixer.Sound(os.path.join("assets/audio/fart_fx","fart_4.wav")),
    pygame.mixer.Sound(os.path.join("assets/audio/fart_fx","fart_5.wav")),
    ]
...
class PowerUp:
    def __init__(self, x, y, starty, up_down, available_images = POWERUP_AVIALABLE, collide_images = POWERUP_COLLIDE, speed = 3, collide = False, img_timer=0):
        self.x = x
        self.y = y
        self.starty = starty
...
class ButtFly:
    def __init__(self, x, y, up_down = "up", health = 100, vel = 5, img_counter = 0, shot = False):
        self.x = x
        self.y = y
        self.up_down = up_down
        self.health = health
...
class Player:
    ...
    def draw(self, window):
        ...
        if self.state == "flying":
            ...
            if self.flycount +1 >= 49:
                self.flycount = 0
            
            self.image = FLAPPING_BIRD[self.flycount//7]
...
def spawn_powerup(powerup_list, enemy_list, powerup_timer):
    if len(powerup_list) > 1:
        for item in powerup_list:
            if item.x < -100 or (item.collide == True and item.img_timer > 32):
